...
def run_queries(server,prometheus_url_1=None, prometheus_url_2=None):
    ...
    aws_response = requests.get(prometheus_url_2, headers={'Authorization': f'Bearer {OC_AUTH_TOKEN}' }, params=params2, verify=False)
...
def main(argv):
    ...
    queries_map = {
            ...
            "db_size": "count(kube_pod_info{pod=~\"kruize-recommendations-[^-]*-[^-]*$\"})",
            "total_exp": "count(kube_pod_info{pod=~\"kruize-recommendations-[^-]*-[^-]*$\"})"
        }
