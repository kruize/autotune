...
kubernetes_object_type = ["deployment", "xyz", "deploymentConfig", "statefulset", "daemonset", "replicaset", "replicationController"]
...
def increment_timestamp(input_timestamp, minutes):
...
def create_update_results_jsons(csv_file_path, split = False, split_count = 1, json_dir = "/tmp/result_jsons", total_exps = 10):
    ...
    with open(csv_file_path, 'r') as csvfile:
        ...
        for row in csvreader:
            ...
            if split == True:
                ...
                # Write the final JSON data to the output file
                result_json_file = json_dir + "/result_" + str(j) + ".json"
                j += 1
                with open(result_json_file, "w") as json_file:
