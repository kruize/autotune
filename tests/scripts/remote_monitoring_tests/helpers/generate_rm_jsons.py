...
def create_exp_jsons_with_mutiple_clusters(split = False, split_count = 1, exp_json_dir = "/tmp/exp_jsons", total_exps = 10, num_clusters = 1):
    complete_json_data = []
    single_json_data = []
    multi_json_data = []
    ...
    for i in range(num_clusters):
        ...
        for j in range(total_exps):
            ...
            # Create a dictionary to hold the container information
            container1 = {
                "container_image_name": container_image_name + "_" + str(cluster_num) + "_" + str(exp_num),
                "container_name": container_name + "_" + str(cluster_num) + "_" + str(exp_num),
            }

            containers.append(container1)

            container2 = {
                "container_image_name": db_container_image_name + "_" + str(cluster_num) + "_" + str(exp_num),
                "container_name": db_container_name + "_" + str(cluster_num) + "_" + str(exp_num),
            }
            ...
            kubernetes_objects = [{
                "type": kubernetes_object_type[type_index],
                "name": kubernetes_object_name + "_" + str(cluster_num) + "_" + str(exp_num),
                "namespace": kubernetes_object_namespace + "_" + str(cluster_num) + "_" + str(exp_num),
                "containers": containers
            }]
            ...
            experiment = {
                "version": "v2.0",
                "experiment_name": exp_name + "_" + str(cluster_num) + "_" + str(exp_num),
                "cluster_name": cluster_name + "_" + str(cluster_num),
                "performance_profile": performance_profile,
                "mode": mode,
                "target_cluster": target_cluster,
                "kubernetes_objects": kubernetes_objects,
            ...
            complete_json_data.append(experiment)
            if split == True:
                if j == 0 or j % split_count != 0:
                    multi_json_data.append(experiment)
                else:
                    exp_json_file = exp_json_dir + "/create_exp_" + str(i) + "_" + str(k) + ".json"
                    with open(exp_json_file, "w") as json_file:
                        json.dump(multi_json_data, json_file, indent=4)
...
def create_update_results_jsons_with_multiple_clusters(csv_file_path, split = False, split_count = 1, json_dir = "/tmp/result_jsons", total_exps = 10, num_res = None, new_timestamp = None , num_clusters = 1):
    # Define the list that will hold the final JSON data
    complete_json_data = []
    single_row_json_data = []
    multi_row_json_data = []
            ...
            for cluster_num in range(num_clusters):
                for exp_num in range(total_exps):
                    ...
                    if split == True:
                        ...
                        # Write the final JSON data to the output file
                        if row_counter == 0:
                            result_json_file = json_dir + "/result_" + str(cluster_num) + "_" + str(exp_num) + ".json"
                            with open(result_json_file, "w") as json_file:
                                json.dump(single_row_json_data, json_file, indent=4)
                ...
                if len(single_row_json_data) > 0:
                    for k in single_row_json_data:
                        exisiting_result_json_file = json_dir + "/result_" + str(cluster_num) + "_" + str(num_exp) + ".json"
                        with open(exisiting_result_json_file, "r+") as file:
                            # Load the existing result JSON
                            existing_data = json.load(file)
                            # Append the new results object to the existing data
