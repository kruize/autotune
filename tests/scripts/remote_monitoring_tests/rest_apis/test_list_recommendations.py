...
@pytest.mark.extended
@pytest.mark.parametrize("latest", ["true", "false"])
def test_medium_term_list_recommendations_with_only_latest(latest, cluster_type):
    """
    ...
    for i in range(num_exps):
        ...
        if latest == "true":
            update_results_json.append(list_of_result_json_arr[i][len(list_of_result_json_arr[i]) - 1])
            # Expected duration in hours is 672h for medium term plus or minus 30s of data is considered to generate recommendations
            ...
            # Recommendations will be generated when 24h results is available
            num_results_without_recos = int(MEDIUM_TERM_DURATION_IN_HRS_MAX * 4 - 1)
            for j in range(num_results_without_recos, total_num_results):
                update_results_json.append(list_of_result_json_arr[i][j])
...
@pytest.mark.extended
@pytest.mark.parametrize("latest", ["true", "false"])
def test_long_term_list_recommendations_with_only_latest(latest, cluster_type):
    """
        Test Description: This test validates medium term list recommendations for multiple experiments posted using different json files
                          and query with only the parameter latest and with both latest=true and latest=false
    """
    input_json_file = "../json_files/create_exp.json"
    result_json_file = "../json_files/update_results.json"
    test_name = "test_long_term_list_recommendations_with_only_latest"
    ...
    # Validate the json against the json schema
    errorMsg = validate_list_reco_json(list_reco_json, long_term_list_reco_json_schema)
    assert errorMsg == ""
    for i in range(num_exps):
        create_exp_json_file = "/tmp/create_exp_" + str(i) + ".json"
        update_results_json_file = "/tmp/update_results_" + str(i) + ".json"
        ...
        if latest == "true":
            update_results_json.append(list_of_result_json_arr[i][len(list_of_result_json_arr[i]) - 1])
            # Expected duration in hours is 1440h for long term plus or minus 30s of data is considered to generate recommendations
            ...
            # Recommendations will be generated when 24h results is available
            num_results_without_recos = int(LONG_TERM_DURATION_IN_HRS_MAX * 4 - 1)
            for j in range(num_results_without_recos, total_num_results):
                update_results_json.append(list_of_result_json_arr[i][j])
