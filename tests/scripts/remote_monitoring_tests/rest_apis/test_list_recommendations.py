...
def test_invalid_list_recommendations_notification_codes(cluster_type: str):
    ...
    for i in range(num_exps):
        ...
        print("delete exp = ", response.status_code)
@pytest.mark.negative
def test_list_recommendations_invalid_cluster(cluster_type):
    """
    Test Description: This test validates listRecommendations by passing an invalid cluster name
    """
    input_json_file = "../json_files/create_exp.json"
    ...
    print(data)
    assert response.status_code == ERROR_STATUS_CODE
    INVALID_CLUSTER_NAME_MSG = "Given cluster name - \" " + cluster_name + " \" is not valid"
    assert data['message'] == INVALID_CLUSTER_NAME_MSG, f"expected - {INVALID_CLUSTER_NAME_MSG}, actual - {data['message']}"
...
@pytest.mark.sanity
@pytest.mark.parametrize("latest", ["true", "false"])
def test_list_recommendations_single_cluster_and_latest(latest, cluster_type):
    """
    Test Description: This test validates listRecommendations by passing a valid cluster name and latest as true or false
    """
    input_json_file = "../json_files/create_exp.json"
...
@pytest.mark.negative
@pytest.mark.parametrize("monitoring_end_time", ["2022-12-20T21:10:11", "20220211"])
def test_list_recommendations_cluster_name_and_monitoring_end_time_invalid(monitoring_end_time, cluster_type):
    """
    Test Description: This test validates listRecommendations by passing a valid cluster name and an invalid monitoring end time value
    """
    input_json_file = "../json_files/create_exp.json"
...
@pytest.mark.extended
def test_list_recommendations_single_cluster_multiple_exps(cluster_type):
    """
    Test Description: This test validates list recommendations for a single cluster with multiple experiments posted using different json files
    """
    num_clusters = 1
    num_exps = 6
    num_res = 100
    ...
    # Create the create experiment jsons
    create_exp_jsons_with_mutiple_clusters(split, split_count, exp_jsons_dir, num_exps, num_clusters)

    # Create the update result jsons
    create_update_results_jsons_with_multiple_clusters(metrics_csv, split, split_count, result_jsons_dir, num_exps, num_res, num_clusters=num_clusters)
...
@pytest.mark.extended
def test_list_recommendations_multiple_clusters_multiple_exps(cluster_type):
    """
    Test Description: This test validates list recommendations for multiple clusters with multiple experiments posted using different json files
    """
    num_clusters = 6
    num_exps = 6
    num_res = 120
