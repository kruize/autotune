...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(argv,"h:c:s:d:t:q:p:r:o:")
    except getopt.GetoptError:
        print("kruize_metrics.py -c <cluster type> -s <server>")
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-h':
            print("kruize_metrics.py -c <cluster type> -s <server> -t <time duration for a query in mins:Default:60m> -o <output dir:Default:./results> -d <duration the script runs in hours:Default:None> -q <query_type:increase/total.Default:increase>")
            sys.exit()
        elif opt == '-c':
            cluster_type = arg
        elif opt == '-s':
