...
function aks_deploy() {
	...
	sed -i'' -e "s|{{ HPO_IMAGE }}|${HPO_DOCKER_IMAGE}|" "${AUTOTUNE_DEPLOY_MANIFEST}"
...
function check_prometheus_installation() {
	echo
	echo "Info: Checking pre requisites for AKS..."
	check_kustomize

	kubectl_cmd="kubectl"
	prometheus_pod_running=$(${kubectl_cmd} get pods --all-namespaces | grep "prometheus-k8s-1")
	if [ "${prometheus_pod_running}" == "" ]; then
		echo "Prometheus is not running, use 'scripts/prometheus_on_aks.sh' to install."
		exit 1
	fi
...
function aks_terminate() {
	# If autotune_ns was not set by the user
	if [ -z "$autotune_ns" ]; 	then
		autotune_ns="monitoring"
	fi

	echo -n "###   Removing autotune for AKS"
