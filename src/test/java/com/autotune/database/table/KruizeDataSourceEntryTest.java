...
public class KruizeDataSourceEntryTest {
    ...
    @Test
    @DisplayName("Test filtering of invalid cluster names - empty strings")
    public void testFilteringEmptyClusterNames() {
        KruizeDataSourceEntry entry = new KruizeDataSourceEntry();
        List<String> clusters = Arrays.asList("valid-cluster", "", "  ", "another-valid");
    ...
    @Test
    @DisplayName("Test filtering of invalid cluster names - exceeding max length")
    public void testFilteringTooLongClusterNames() {
        KruizeDataSourceEntry entry = new KruizeDataSourceEntry();
        String tooLongName = "a".repeat(254); // 254 characters, exceeds max of 253
        String validName = "a".repeat(253);   // 253 characters, at the limit
        List<String> clusters = Arrays.asList("short-name", tooLongName, validName);
        
        entry.setClusterList(clusters);
        
        // Too long cluster name should be filtered out
        List<String> result = entry.getClusterList();
        assertEquals(2, result.size());
        assertTrue(result.contains("short-name"));
        assertTrue(result.contains(validName));
        assertFalse(result.contains(tooLongName));
    }

    @Test
    @DisplayName("Test filtering with mixed valid and invalid cluster names")
    public void testFilteringMixedValidInvalidClusterNames() {
        KruizeDataSourceEntry entry = new KruizeDataSourceEntry();
        ...
        // Only valid cluster names should remain
        List<String> result = entry.getClusterList();
        assertEquals(3, result.size());
        assertEquals("cluster1", result.get(0));
        assertEquals("cluster2", result.get(1));
    ...
    @Test
    @DisplayName("Test setClusters with comma-separated string containing invalid names")
    public void testSetClustersStringWithInvalidNames() {
        KruizeDataSourceEntry entry = new KruizeDataSourceEntry();
        String tooLong = "y".repeat(300);
        // Set comma-separated string with mix of valid and invalid
        entry.setClusters("valid1,," + tooLong + ",valid2,   ,valid3");
        
        // Only valid cluster names should be parsed
        List<String> result = entry.getClusterList();
        assertEquals(3, result.size());
        assertTrue(result.contains("valid1"));
        assertTrue(result.contains("valid2"));
        assertTrue(result.contains("valid3"));
    }
}
